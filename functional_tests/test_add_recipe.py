...
from meals.models import Foods, Servings, Ingredients, FoodNotes, FoodGroup, FoodType
...
class AddIngredientRecipeTest(FunctionalTest):
    def this_setup(self):
        ...
        # create FoodGroups
        self.my_meals_food_group = FoodGroup.objects.create(name="My Meals", rank=1)

        self.my_recipe_food_group = FoodGroup.objects.create(name="My Recipes", rank=2)
        self.veg_food_group = FoodGroup.objects.create(name="Vegatables", rank=3)

        self.meat_food_group = FoodGroup.objects.create(name="Meat", rank=4)
        ...
        self.ice_cream = Foods.objects.create(
            name="ice cream",
            food_group=self.my_meals_food_group,
        ...
        self.carrots = Foods.objects.create(
            name="carrots",
            food_group=self.veg_food_group,
            cals_per_gram=Decimal(5.9),
            fat_per_gram=Decimal(4.491),
            carbs_per_gram=Decimal(0.8732),
            sugar_per_gram=Decimal(0.8732),
        ...
        self.chicken = Foods.objects.create(
            name="chicken",
            food_group=self.meat_food_group,
            cals_per_gram=Decimal(5.9),
            fat_per_gram=Decimal(4.491),
            carbs_per_gram=Decimal(0.8732),
            sugar_per_gram=Decimal(0.8732),
    ...
    def test_add_ingredient_recipe(self):
        ...
        self.check_element_content(
            'label[for="add-recipe-filters"]', "css", "text", "Search Filters"
        )

        filters = [
            e.get_attribute("value")
            for e in self.browser.find_elements_by_class_name("filter")
        ]
        ...
        save_button = self.browser.find_element_by_id("add-recipe-save-button")
        save_button.click()
        ...
        recipe_name_input = self.browser.find_element_by_id("add-recipe-recipe-name")
        ...
        self.check_element_content(
            ...
            "id",
            "text",
            "Recipes require more than one ingredient",
        )
        self.check_element_content("add-recipe-name-errors", "id", "text", "")
        ...
        self.check_element_content(
            "add-recipe-ingredients-container-errors", "id", "text", ""
        )

        time.sleep(1)
        self.check_element_content(
            "add-recipe-save-status", "id", "text", "Recipe Saved"
        )
        ...
        ingredients = self.browser.find_elements_by_class_name("add-recipe-ingredient")
        ...
        self.browser.find_elements_by_class_name("search-result__button")[0].click()
