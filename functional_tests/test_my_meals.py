...
from meals.models import Foods, Servings, Ingredients, FoodNotes, FoodType, FoodGroup
...
class MyMealTests(FunctionalTest):
    ...
    def create_meals(self, user):
        ...
        self.ham_sandwich = Foods.objects.create(
            name="Ham Sandwich", date=first_date, user=user, food_group=meal_group
        )
    ...
    def test_delete_meal(self):
        ...
        confirmation = self.browser.find_element_by_id("my-meals-delete-confirmation")
    ...
    def test_my_meals(self):
        ...
        self.assertEqual(
            len(
                [
                    ...
                    if "pretzel" in r.text.lower() or "cheese" in r.text.lower()
                ]
            ),
            1,
        )  # should be one because one 'Pretzel and Cheese' has no user
        ...
        self.check_element_content(
            "my-meals-modal-notes-body", "id", "text", self.pretzels_cheese_notes.notes
        )
